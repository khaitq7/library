...
//	private int outputWidthTileSize;
	...
	public PuzzleCreator(Bitmap source, GameBoard board){
		...
		xTileSize = board.getxTileSize();
		yTileSize = board.getyTileSize();
		//outputWidthTileSize = board.getWidthTileSize();
		this.board = board;
	...
	public PuzzleTile[][] createPuzzle(){
		...
		if(sourceImage == null || outputBoardSize == null || xTileSize <= 0 || yTileSize <= 0 )
		...
		Dimension neededRes = new Dimension(outputBoardSize.x*xTileSize,outputBoardSize.y*yTileSize);
		...
		for(int y = 0; y<outputBoardSize.y; y++){
			for(int x = 0; x<outputBoardSize.x; x++){
				...
				if(!(x==outputBoardSize.x-1 && y==outputBoardSize.y-1)){
					Bitmap temp = Bitmap.createBitmap(sourceImage, x*xTileSize, y*yTileSize,
							xTileSize, yTileSize);
